...
- Nested quote issues in here-strings
...
class IssueType(Enum):
    ...
    NESTED_QUOTE_IN_HERESTRING = "nested_quote_in_herestring"
...
class PowerShellSyntaxValidator:
    ...
    def _validate_syntax(self, content: str):
            ...
            while i < len(line):
                ...
                if in_herestring:
                    if line.strip() == herestring_marker:
                        ...
                        # Check for nested quotes in here-strings
                        self._check_nested_quotes_in_herestring(line, line_num)
                    i += 1
                    continue
                
                # Check for here-string start
                if not in_single_quote and not in_double_quote:
                    herestring_match = self._check_herestring_start(line, i)
                    if herestring_match:
                        in_herestring = True
                        herestring_marker = herestring_match
                        i += len(herestring_match) - 1
                        i += 1
                        continue
                ...
                if not in_single_quote and not in_double_quote and not in_herestring:
                    if char in "({[":
        ...
        if in_herestring:
            self.issues.append(ValidationIssue(
                ...
                line=len(lines),
                column=0,
                message=f"Unterminated here-string (expected '{herestring_marker}')",
                context=lines[-1] if lines else ""
            ))
    ...
    def _check_herestring_start(self, line: str, pos: int) -> Optional[str]:
        """Check if a here-string starts at the given position."""
        # Look for @" or @'
        if pos < len(line) - 1 and line[pos] == '@':
            if line[pos + 1] == '"':
                return '@"'
            elif line[pos + 1] == "'":
                return "@'"
    ...
    def _check_nested_quotes_in_herestring(self, line: str, line_num: int):
        """Check for problematic nested quotes in here-strings."""
        # Look for potential issues like unescaped quotes that might cause problems
        for i, char in enumerate(line):
            if char in ['"', "'"]:
                # This is more of a warning than an error
                if i == 0 or line[i-1] != '`':  # Not escaped
                    self.issues.append(ValidationIssue(
                        type=IssueType.NESTED_QUOTE_IN_HERESTRING,
                        line=line_num,
                        column=i + 1,
                        message=f"Unescaped quote '{char}' in here-string might cause issues",
                        context=self._get_context(line, i)
                    ))
...
def main():
    ...
    print(f"PowerShell Syntax Validator")
