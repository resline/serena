...
function Test-PipInstallationWindows10 {
    ...
    try {
        ...
        Write-Host "  ⚠ Pip module test failed" -ForegroundColor Yellow
    }
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  ✓ Pip import successful: $importTest" -ForegroundColor Green
            return $true
        }
    }
    catch {
        Write-Host "  ⚠ Pip import test failed" -ForegroundColor Yellow
    }
    
    # Test 3: Repair attempt for Windows 10
    Write-Host "  🔧 Attempting Windows 10 pip repair..." -ForegroundColor Yellow
    try {
        $repairArgs = @(
        ...
        if ($LASTEXITCODE -eq 0) {
            Write-Host "  ✓ Pip repair successful: $finalTest" -ForegroundColor Green
            return $true
        }
    }
    catch {
        Write-Host "  ❌ Pip repair failed" -ForegroundColor Red
    }
...
function Remove-TemporaryFilesWindows10 {
    ...
    foreach ($pattern in $Patterns) {
        try {
            ...
            Write-Host "  ⚠ Could not remove some $pattern files: $($_.Exception.Message)" -ForegroundColor Yellow
        }
    }
}
...
function Test-PackageIntegrityWindows10 {
    ...
    if (Test-Path $sitePackages) {
        ...
        if ($packages.Count -gt 5) {  # Expect several packages
            ...
            Write-Host "  ✓ Dependencies: $($packages.Count) packages installed" -ForegroundColor Green
        } else {
    ...
    if (Test-Path $languageServers) {
        ...
        if ($servers.Count -gt 3) {  # Expect several language servers
            ...
            Write-Host "  ✓ Language Servers: $($servers.Count) servers available" -ForegroundColor Green
        } else {
    ...
    if (Test-Path $mainScript) {
        ...
        Write-Host "  ✓ Scripts: Main launcher created" -ForegroundColor Green
    } else {
    ...
    if ($validationResults.OverallValid) {
        ...
        Write-Host "⚠️ Package validation issues found:" -ForegroundColor Yellow
