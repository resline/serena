...
foreach ($file in $powershellFiles) {
    ...
    if ($lineEndingResult) {
        if ($lineEndingResult.HasCorrectEndings) {
            Write-Host "  [OK] Line endings: CRLF ($($lineEndingResult.CRLFCount) lines)" -ForegroundColor Green
        }
        else {
            Write-Host "  [X] Line endings: PROBLEMATIC" -ForegroundColor Red
        ...
        Write-Host "  [X] Line endings: Could not analyze" -ForegroundColor Red
        $fileValid = $false
    }
    
    if ($encodingResult) {
        if ($encodingResult.IsUTF8Compatible) {
            Write-Host "  [OK] Encoding: $($encodingResult.Encoding)" -ForegroundColor Green
        }
        else {
            Write-Host "  [X] Encoding: $($encodingResult.Encoding)" -ForegroundColor Red
            $fileValid = $false
        }
    }
    else {
        Write-Host "  [X] Encoding: Could not analyze" -ForegroundColor Red
        $fileValid = $false
    }
    ...
    if ($syntaxResult) {
        if (-not $syntaxResult.HasSyntaxErrors) {
            ...
            Write-Host "  [X] PowerShell syntax: $($syntaxResult.ErrorCount) error(s)" -ForegroundColor Red
            if ($Verbose) {
        ...
        Write-Host "  [X] PowerShell syntax: Could not analyze" -ForegroundColor Red
        $fileValid = $false
    }
